...
if [ "x$1" = "xmac" ]; then
    ...
    if [ -n "$START_TS" ]; then
        data=$(/usr/bin/nice -n 19 /usr/bin/log show --style "$STYLE" --info --predicate "$PRED" --start "$START_TS")
    else
        WINDOW="${2:-10m}"
        data=$(/usr/bin/nice -n 19 /usr/bin/log show --style "$STYLE" --info --predicate "$PRED" --last "$WINDOW")
    fi
